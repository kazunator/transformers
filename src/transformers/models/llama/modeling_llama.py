...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids=None, unsqueeze_dim=1):
    ...
    This version is adapted for int8 quantization and float16 precision.
    """
    # Ensure cos and sin are in float16
    cos = cos.to(torch.float16).unsqueeze(unsqueeze_dim)
    sin = sin.to(torch.float16).unsqueeze(unsqueeze_dim)

    # Dequantize q and k if they are in int8
    if q.dtype == torch.int8:
        q_scale = q.abs().amax(dim=-1, keepdim=True) / 127
        q_scale = torch.clamp(q_scale, min=1e-6)
        q = q.to(torch.float16) * q_scale
    else:
        q = q.to(torch.float16)

    if k.dtype == torch.int8:
        k_scale = k.abs().amax(dim=-1, keepdim=True) / 127
        k_scale = torch.clamp(k_scale, min=1e-6)
        k = k.to(torch.float16) * k_scale
    else:
    ...
    # Requantize to int8 if original inputs were int8
    if q.dtype == torch.int8:
        q_embed = (q_embed / q_scale).round().to(torch.int8)
    if k.dtype == torch.int8:
        k_embed = (k_embed / k_scale).round().to(torch.int8)
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        self.o_proj.weight = nn.Parameter(self.o_proj.weight.half())
        if self.o_proj.bias is not None:
            self.o_proj.bias = nn.Parameter(self.o_proj.bias.half())
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.config.pretraining_tp > 1:
            ...
            value_states = self.v_proj(hidden_states.half())
...
class LlamaIntFlashAttention(LlamaAttention):
    ...
    def forward(
        ...
        # Quantize query, key, and value states to int8
...
LLAMA_ATTENTION_CLASSES = {
    ...
    "int_flash_attention": LlamaIntFlashAttention,  # Add this line
}
...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states = (residual + hidden_states).to(torch.float16)
